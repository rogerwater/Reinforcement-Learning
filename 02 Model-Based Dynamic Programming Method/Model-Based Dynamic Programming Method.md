...
\nu_{k+1}(s) = \sum_{a \in A} \pi(a|s) (R^a_s + \gamma \sum_{s' \in S} P^a_{SS'} \nu_k(s'))
$$
⾼斯-赛德尔解线性⽅程组迭代收敛条件是右⾯的未知数矩阵的谱半径⼩于1，这个条件也是由压缩映射定理得来的。在这⾥，我们不去求系数矩阵的谱，⽽是找⼀个特殊的度量 $\rho$ 以便简化证明，这个度量我们选为⽆穷范数，即
$$
||\nu||_{\infty} = \underset{s \in S}{\text{max}} ||\nu(s)||
$$
...
$$
\rho(T^{\pi}(u),T^{\pi}(\nu)) = ||T^{\pi}(u) - T^{\pi}(\nu)||_{\infty} \\
=||(R^{\pi} + \gamma P^{\pi} u) - (R^{\pi} + \gamma P^{\pi} \nu)||_{\infty} \\
=||\gamma P^{\pi}(u - \nu)||_{\infty} \\
\le ||\gamma P^{\pi}||(u - \nu)||_{\infty}||_{\infty} \\
\le \gamma ||(u - \nu)||_{\infty}
$$
因为 $0 \le \gamma < 1$ ，所以 $T^{\pi}(\nu)$ 是一个压缩映射，该迭代序列最终收敛到相应于策略 $\pi$ 的值函数。
